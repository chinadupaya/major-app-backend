...
const mainController = (repo, socket) =>{
    const controller = {
        getUsers: (req, res) => {
            repo.getUsers()
                .then((response) => {
        ...
        postUserRoom:async (req,res)=>{
            data = req.body;
            var roomExistsPromise = await repo.checkRoomExists(data.userId, data.chatWith);
            if (roomExistsPromise.length == 0){
                var id = shortid.generate()
                console.log("no room exists");
                var promises = [];
                promises.push(repo.postRoom(id, data.userId, data.chatWith,data.nameOne,data.nameTwo));
                //id,roomId, sentBy,content, type
                var messageId = shortid.generate();
                //id, roomId, type, sentBy,content
                promises.push(repo.postMessage(messageId, id, "text","test-message",`start of chat ${id}`));
                ...
                .then((response)=>{
                    return res.status(200).json({
                        data: {
                            ...
                            user_name: data.nameOne,
                            chat_with_name: data.nameTwo
                        }
                    })
                })
                .catch((err)=>{
        ...
        getUserRooms: (req,res)=>{
            repo.getUserRooms(req.params.userId)
            .then((response)=>{
                return res.status(200).json({
                    data:response
                })
        ...
        postMessage: (req,res)=>{
            ...
            repo.postMessage(id, data.roomId, "text", data.sentBy,data.content);
            var messageObj = {
                room_id: data.roomId,
                content: data.content,
                sent_by: data.sentBy
            }
            socket.in(data.roomId).emit('newMessage',JSON.parse(JSON.stringify(messageObj)));
        },
        getMessages: (req,res)=>{
            repo.getMessages(req.params.roomId)
            .then((response)=>{
                return res.status(200).json({
                    data: response
                })
            })
            //console.log(Socket().previousId);
        }
